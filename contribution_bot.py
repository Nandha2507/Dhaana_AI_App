...
BOT_DISPLAY_NAME = "Contribution Bot"  # Change bot display name here

# Admin Telegram user IDs
ADMINS = [7854166119,1255111776]  # Replace with actual admin Telegram IDs
...
async def start(update: Update, context) -> int:
    ...
    context.user_data['username'] = username  # Save username in session
...
async def contribution_type_selected(update: Update, context) -> int:
    ...
    if contribution_type == "self":
        ...
        context.user_data['family_members'] = []  # Initialize list
...
async def amount_received(update: Update, context) -> int:
    try:
        ...
        contribution_type = context.user_data.get('contribution_type')
        ...
        if contribution_type == 'family':
            ...
            await update.message.reply_text(f"Amount {amount} recorded. Please upload the screenshot of your contribution:")
            return SCREENSHOT
...
async def screenshot_received(update: Update, context) -> int:
    if update.message.photo:
        ...
        if context.user_data['contribution_type'] == 'family':
            ...
            await update.message.reply_text(
                f"✅ Contribution recorded successfully!\n"
                f"Summary:\n"
...
async def more_family_handler(update: Update, context) -> int:
    ...
    if query.data == "more_yes":
        ...
        summary = "✅ All family contributions recorded successfully!\nSummary:\n"
        for m in context.user_data['family_members']:
            summary += f"Member: {m['family_member_name']}, Amount: {m['amount']}, Screenshot: Saved\n"
        summary += "\nType /start to add another contribution."
...
async def export_excel(update: Update, context):
    ...
    if not contributions:
        await update.message.reply_text("No contributions found in the database.")
        return
    ...
    export_path = os.path.join("exports", f"contributions_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx")
    df.to_excel(export_path, index=False)

    await update.message.reply_text(f"✅ Contributions exported successfully!\nFile: {export_path}")

    ...
    if update.message.photo:
        ...
        if context.user_data['contribution_type'] == 'family':
            ...
            await update.message.reply_text(
                ...
                f"Month: {month}\n"
                f"Type: {context.user_data['contribution_type']}\n"
                f"Member: Self\n"
...
async def more_family_handler(update: Update, context) -> int:
    ...
    if query.data == "more_yes":
        ...
        # Show summary of all family members
        summary_text = f"✅ All family contributions recorded successfully!\n\nSummary:\n"
        for idx, member in enumerate(context.user_data['family_members'], 1):
            summary_text += (
                f"{idx}. Member: {member['family_member_name']}, "
...
def main():
    ...
    logger.info(f"Starting bot webhook on port {port}, URL: {webhook_url}")
